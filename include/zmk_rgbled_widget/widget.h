...
#define SHOW_BLE_PROFILE (IS_ENABLED(CONFIG_RGBLED_WIDGET_SHOW_BLE_PROFILE)) &&                    \
        (!IS_ENABLED(CONFIG_ZMK_SPLIT) || IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL))
...
// Mono LED API (available when mono mode is enabled)
#if IS_ENABLED(CONFIG_RGBLED_WIDGET_MONO)

// Mono LED status types
enum mono_led_status {
    MONO_LED_STATUS_BATTERY = 0,
    MONO_LED_STATUS_CONNECTIVITY = 1,
    MONO_LED_STATUS_LAYER = 2
};

// Mono LED priority levels
enum mono_led_priority {
    MONO_LED_PRIORITY_CONNECT = 0,  // Highest - connectivity status
    MONO_LED_PRIORITY_BATTERY = 1,  // Medium - battery status
    MONO_LED_PRIORITY_LAYER = 2     // Lowest - layer status
};

// Mono LED control functions
int mono_led_request_status(enum mono_led_status status, uint32_t blink_interval_ms,
                           uint32_t timeout_ms, bool persistent);
int mono_led_clear_status(enum mono_led_status status);
int mono_led_clear_all(void);

// Mono LED status query functions
bool mono_led_is_active(enum mono_led_status status);
uint32_t mono_led_get_timeout(enum mono_led_status status);

// Utility functions for mono LEDs
const char* mono_led_get_status_name(enum mono_led_status status);
bool mono_led_is_available(void);
